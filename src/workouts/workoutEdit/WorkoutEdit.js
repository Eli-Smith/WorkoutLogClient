...
const WorkoutEdit = (props) => {
    ...
    const [editDef, setEditDef] = useState(props.workoutToUpdate.definiton);
    const [editRes, setEditRes] = useState(props.workoutToUpdate.result);

    const workoutUpdate = (event, workout) => {
        event.preventDefault();
        
        fetch(`http://localhost:3000/api/log/updatelog/${props.workoutToUpdate.id}`, {
            method: 'PUT',
            ...
            props.fetchWorkouts();
            props.updateOff()
            console.log('Use State Values', editDef, editDesc, editRes)
        })
    }
        ...
        <Modal isOpen={true}>
            ...
            <ModalBody>
                <Form onSubmit={workoutUpdate}>
                    ...
                    <FormGroup>
                        <Label htmlFor='definition' value={editDef} onChange={(e) => setEditDef(e.target.value)}>Edit Definition:</Label>
